   ...
   for(int i = 0; i < 16; i++){
    ...
    vec4 spotLightResult;
     for(int i = 0; i < 16; i++){
      ...
      vec2 waterUV = frag_UV + (time * 0.1);
      vec4 mapColor = texture(mapTexture, fragmentMapUV);
      vec4 grassColor = texture(grassTexture, frag_UV);
      vec4 waterColor = texture(waterTexture, waterUV);
      vec4 rockColor = texture(rockTexture, frag_UV);
      vec4 sandColor = texture(sandTexture, frag_UV);

      waterColor = mix(grassColor, waterColor, mapColor.b);
      sandColor = mix(waterColor, sandColor, mapColor.r);
      rockColor = mix(rockColor, sandColor, mapColor.a);

      color =  rockColor * (spotLightResult + pointLightResult + directionalLightResult) * objectColor;  

      //color = vec4(frag_Normal,0);
